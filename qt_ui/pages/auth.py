...
class AuthPage(QWidget):
    def __init__(self, project_dir: Path, db: Database) -> None:
        ...
        for name in ('profileCard', 'browserCard'):
            require(self, QFrame, name).setProperty('card', True)
        for name in ('profileLayout', 'browserLayout'):
            layout = require(self, QVBoxLayout, name)
            layout.setContentsMargins(18, 16, 18, 16)
            layout.setSpacing(10)
        require(self, QLabel, 'profileTitle').setProperty('cardTitle', True)
        require(self, QLabel, 'browserTitle').setProperty('cardTitle', True)
        require(self, QLabel, 'statusTitle').setProperty('cardTitle', True)
