...
class ExecutionPage(QWidget):
    ...
    def __init__(self, project_dir: Path, db: Database) -> None:
        ...
        for card in (summary_card, progress_card):
            card.setProperty('card', True)
        summary_card.setProperty('executionSummary', True)
        summary_card.setFixedHeight(58)
        progress_card.setProperty('executionProgress', True)
        summary_layout = require(self, QHBoxLayout, 'summaryLayout')
        summary_layout.setContentsMargins(12, 6, 10, 6)
        ...
        self.status.setProperty('statusBadge', True)
        ...
        self.target_summary.setProperty('summaryValue', True)
        require(self, QLabel, 'sessionLabel').setProperty('muted', True)
        self.session_spin = require(self, QSpinBox, 'sessionSpin')
        require(self, QLabel, 'sessionLabel').setText('同時セッション数')
        self.session_spin.setFixedWidth(104)
        ...
        self.progress_count.setProperty('progressCount', True)
        require(self, QLabel, 'progressLabel').setProperty('muted', True)
        self.status_tab = require(self, QPushButton, 'statusTabButton')
        self.log_tab = require(self, QPushButton, 'logTabButton')
        for button in (self.status_tab, self.log_tab):
            button.setProperty('executionTab', True)
