...
クリックや入力などでは、「操作対象を選択」を押して Chrome を開きます。画面は最初は待機状態です。F1 を押してから範囲をクリックするとパス条件を追加し、F2 を押してから対象をクリックすると確定します。選択後は待機状態へ戻るため、複数条件では F1 とクリックを繰り返します。Esc で選択内容を保ったまま待機状態へ戻れるため、画面を操作してから選択を再開できます。登録後は「このイベントを試す」で単独動作を確認できます。

別テーブルへ同じ行番号を対応させる場合は、基準行内の文字を F1 で選択し、F2 で別テーブルの対象を選択します。同じ行内で複数回 F1 を押すと各条件を AND で結合でき、共通の外側範囲は不要です。必要な場合は、その前に外側範囲を F1 で追加できます。実行時に基準行の index を再計算するため、固定行番号は保存しません。
